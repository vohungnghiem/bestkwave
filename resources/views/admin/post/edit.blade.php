...
<div class="content-wrapper">
    ...
    <section class="content">
        <form method="POST" action="{{url('admincp/post/'.$post->id.'/update')}}" autocomplete="off" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select class="form-control select2bs4" name="category" style="width: 100%;">
                                    {{showCategoriesSelected($categories,0,'',$post->category)}}
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select2bs4" name="type_post" style="width: 100%;">
                                    @foreach ($typeposts as $item)
                                        <option value="{{$item->id}}"  @if($item->id == $post->type_post) selected @endif>{{$item->title}}</option>                                        
                                    @endforeach
                                </select>
                            </div>
                            <label> Thumb</label>
                            <div class="text-center">
                                <label class="cabinet">
                                    <figure>                
                                        <img  @if($post->image) src="{{'public/uploads/thumb/'.year($post->created_at).'/'.month($post->created_at).'/'.$post->image}}?rand={{md5(time())}}" @else src="public/admin/images/non_image.png" @endif
                                        ...
                                        <input type="hidden" class="avatar file center-block" name="image" value="{{$post->image}}" />
                                    </figure>     
                                </label>
                            </div>  
                            <div class="form-group">
                                <label for="title"> Title </label>
                                <input type="text" name="title" value="{{$post->title}}" class="form-control @if ($errors->first('title')) is-invalid @endif" id="title" placeholder="title" required >
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="caption" class="form-control" rows="5">{{$post->caption}}</textarea>
                            </div>
                            {{-- <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="checkbox" name="status" @if ($post->status == 1) checked @endif data-bootstrap-switch data-on-color="success">
                            </div> 
                            <div>
                                <button class="btn btn-primary" type="submiti"> Confirm </button>
                                <button class="btn btn-secondary"type="reset"> Cancel </button>
                                <a class="btn btn-dark" href="admincp/post"> Back list </a>
                            </div>  
                        </div>
                    </div>   
                    <div class="card">
                        ...
                        <div class="card-body">
                            @foreach ($socials as $item)
                                <div class="form-group">
                                    ...
                                    <input type="text" name="social[{{$item->id}}]" @foreach ($get_social as $social) @if ($social->id == $item->id) value="{{$social->link}}" @endif @endforeach  class="form-control"> 
                                </div>
                            @endforeach
                        </div>
                    </div>
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="key" value="{{$post->key}}" class="form-control">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <textarea name="content" class="form-control textarea-400">{{$post->content}}</textarea>
                            </div>
...
@section('javascript')
    ...
    <script>cutimage(400,270)</script>
@endsection